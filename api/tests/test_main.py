...
def test_do_not_update_user_with_already_used_username(client, user_db, user_request):
    user_request["username"] = "JaneDoe"
    response = client.put(f"/users/{user_db.id}", json=user_request)
    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_do_not_update_user_with_already_used_email(client, user_db, user_request):
